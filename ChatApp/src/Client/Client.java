...
public class Client {
    ...
    // Регистрация имени пользователя со стороны клиентского приложения
    ...
    // Отправка сообщения, предназначенного для других пользователей на сервер
    ...
    // Принятие с сервера собщение от других клиентов
    protected void receiveMessageFromServer() {
        while (isConnect) {
            try {
                ...
                // Если сообщение с типом ME_ADDED добавляем сообщение в окно переписки
                if (message.getTypeMessage() == MessageType.ME_ADDED) {
                    gui.addMessage(message.getTextMessage());
                    gui.refreshListUsers(model.getUsers());
                }
    ...
    // Отключение нашего клиента от чата
